...
async def analyze_single_match(match: dict) -> bool:
    try:
        ...
        log.error(f"Failed to analyze {match['home']} vs {match['away']}: {e}", exc_info=True)
        return False
...
async def daily_pipeline():
    ...
    try:
        ...
        if not matches:
            ...
            log.info("=== Pipeline done: no matches ===")
            return
        ...
        await send_error_notification(f"Error crítico en el pipeline: {e}")
